<template>
  <div id="page-3">
    ...
    <div id="station-box" class="mt70 flex" style='position: relative;'>
     <div class="chart-ftt-trend">
        <span class="border row1"></span>
        <span class="border row2"></span>
        <span class="border col1"></span>
        <span class="border col2"></span>
        <div class="title">
          <label><span class="title-icon title-icon4"></span>
          <span>{{isEnglish?'FIRST TIME THROUGH': '一次点火成功率'}}</span></label>
          <!-- <img src="../../../static/imgs/export.png" class="export" @click="doPrint()"/> -->
        </div>
        <div id="print-content">
          <ve-line :data="chartData" 
            :extend="extend" 
            :mark-line="markLine"></ve-line>
        </div>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handlerProjectCompare() {
      this.$http.get('/fttApi/pqcMesPoint/projectCompareList?prjCode=&prjStep=&startTime='+this.dateFormate(new Date().setMonth(new Date().getMonth()-1))+'&endTime='+this.dateFormate(new Date())+'&stcode=&prjCodes=&prjSteps=').then(res=>{
   ...
   .chart-ftt-trend {
      ...
      .title {
       padding-top: 13px;
        ...
        .title-icon4{
          background: url("../../../static/imgs/headicon4.png");
        }
      }
      .border {
        position: absolute;
