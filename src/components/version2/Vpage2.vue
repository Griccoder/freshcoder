        ...
        <div class="scroll-box">
          <table class="table">
            ...
            <tr v-for="(item, index) in tableData.varPlanVos"
              ...
              <td class="progress">
                <Progress :num="item.step"></Progress>
              </td>
              <td :class="item.allSuccess?'has-done':'undone'">
                {{item.allSuccess?'已交付':'未交付'}}
              </td>
            </tr>
            </tbody>
          </table>
      ...
      <div class="flex">
        <div class="flex-1 station" v-for="(item,index) in workStationData">
          ...
          <div class="box">
            ...
            <div class="img-box" :class="getImgUrl(item.typeName, currentActions[item.type])">
              ...
              <div class="flex mt5">
                <div class="flex-1 mr10">
                  <!--<el-progress :text-inside="true"-->
                               <!--:stroke-width="14"-->
                               <!--:percentage="parseInt(currentActions[item.type].length/61*100)">-->
                  <!--</el-progress>-->
...
<script>
  import Progress from '../page2/Progress'
  import GLProgress from '../page2/GLProgress.vue'
  export default {
    components: {
      Progress,
      GLProgress
    ...
    methods: {
      ...
      setIconAnimate () {
        let self = this
        if (this.currentActions['FILLING'] &&
          this.currentActions['SWDL'] &&
          this.currentActions['FWP'] &&
          this.currentActions['ROLLER'] &&
        ...
        if (!this.callIsBack) {
          ...
          this.$http.get(url).then(function (res) {
            ...
            if (res.body.status === 200) {
              // console.log(200, res)
              console.log(2000, this.requestId)
              this.tableData = res.body.data
              // this.tableData.varPlanVos = this.filter()
              this.tableData.varPlanVos.map(v => {
                let step = v.carSatauList.lastIndexOf('OK') + 2
                v.step = step
              })
            } else {
      ...
      getImgUrl (name, data) {
        let className
        switch (name) {
          case '加注': if (data) {className='jiazhu2'}else{className='jiazhu1'};
            break;
          case '软件刷写': className='shuaxie';
            break;
          case '四轮定位': if (data) {className='silun2'}else{className='silun1'};
            break;
          case '转毂': if (data) {className='zhuangu2'}else{className='zhuangu1'};
            break;
      ...
      getWorkStation () {
        this.$http.get('/api/plantTypes/list').then(function (res) {
          ...
          this.workStationData.map(v=>{
            switch (v.plant) {
              case 'chengdu.gaoyuan': this.factoryName = '成都高原汽车工业有限公司';
                break;
              case 'hangzhouwan.pilot': this.factoryName = '杭州湾研究院试制车间';
                break;
            }
          })
        }, function (err) {
...
<style lang="less" scoped>
  ...
  .work-station{
    ...
    .station{
      ...
      /*&:nth-child(1) .img-box{*/
        /*background: url("../../../static/imgs/jiazhu.png") no-repeat;*/
      /*}*/
      /*&:nth-child(2) .img-box{*/
        /*background: url("../../../static/imgs/xiezai.png") no-repeat;*/
      /*}*/
      /*&:nth-child(3) .img-box{*/
        /*background: url("../../../static/imgs/silun.png") no-repeat;*/
      /*}*/
      /*&:nth-child(4) .img-box{*/
        /*background: url("../../../static/imgs/zhuangu.png") no-repeat;*/
      /*}*/
      /*&:nth-child(5) .img-box{*/
        /*background: url("../../../static/imgs/shuaxie.png") no-repeat;*/
      /*}*/
      &:last-child{
        padding-right: 0;
        .fa-arrow-circle-right{
